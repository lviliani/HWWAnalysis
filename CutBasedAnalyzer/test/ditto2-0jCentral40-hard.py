...
process.cuts = cms.VPSet(
    ...
    cms.PSet(
        name = cms.string('barrel'),
        label = cms.string('eta barrel'),
        cut = cms.string('abs(pA.eta()) <  1.5 && abs(pB.eta()) < 1.5'),
    ),
    cms.PSet(
        name = cms.string('minMet'),
        label = cms.string('min #slash{E}_{T}'),
        cut = cms.string('met > 30'),
    ),
    cms.PSet(
        name = cms.string('mllRange'),
        label = cms.string('mll range'),
        cut = cms.string('mll > 12. && mll < 60.'),
    ),
    cms.PSet(
        name = cms.string('cjv40'),
        label = cms.string('Central Jet Veto 40'),
        cut = cms.string('nCentralJets40 == 0'),
    ),
    cms.PSet(
        name = cms.string('pTtrail'),
        label = cms.string('pT trail'),
        cut = cms.string('pB.pt() > 30'),
    ),

    cms.PSet(
        name = cms.string('dPhi'),
        label = cms.string('dPhi'),
