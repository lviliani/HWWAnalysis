...
//            09/1092011 Pasquale Musella <pasquale.musella@cern.ch>
////////////////////////////////////////////////////////////////////////////////
...
template <>
void ssToVal(std::stringstream& ss,std::string& e){e=ss.str();}
...
//______________________________________________________________________________
CommandLine::CommandLine(): _vecsep(",")
{
...
bool CommandLine::parse(int argc,char**argv)
{
  ...
  for (int i=1;i<argc;i++) {
	...
	return false;
      ...
      _options[opt].first += _vecsep+next;
    }
    else{
...
void CommandLine::print()
{
  ...
  for (StrVec_t::const_iterator itvec=_ordered_options.begin();
    ...
    if (it->second.first.find(_vecsep)<string::npos) {
...
//______________________________________________________________________________
std::map<std::string, std::string> CommandLine::getOptionMap() {
    std::map<std::string, std::string> theMap;
    OptionMap_t::const_iterator it;
    for (it = _options.begin();it!=_options.end();++it)
        theMap[it->first] = it->second.first;
